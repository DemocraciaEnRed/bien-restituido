  ...
  const renderFieldShowCard = (fields, ordering) => {
    ...
    return (
      <span>
        {fields.map((field) => {
          return (
            <span key={field._id} className={`mx-2 ${ordering}`}> <span className="font-bold">{field.name}</span>:{" "} {asset.extras[field.slug]}</span>
          );
        })}
      </span>
    );
  ...
  return (
    ...
    >
        ...
        <CardHeader className="flex flex-row w-full justify-between">
            <div className="flex items-center">
              ...
              <div className="flex flex-col p-2">
                <div>
                  <span className="mr-2"><span className="font-bold">{asset.category.name}:</span>{" "} {asset.subCategory?.name}</span> 
                  {renderFieldShowCard(extraFields.filter((field) => field.showCard === showCardOptions.ALLWAYS.value), '')}
                </div>
                <div>
        ...
        <CardContent className={`${!showMore && "hidden"} w-full`}>
              ...
              <p className="uppercase underline font-bold mt-5">Información del destino</p>
              {/* {renderFieldShowCard(extraFields.filter((field) => field.showCard === showCardOptions.EXPANDED.value), 'flex')} */}             
        </CardContent>        
